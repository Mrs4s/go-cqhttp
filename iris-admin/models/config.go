...
func paraseConfig(configs []QQConfig) (*config2.Config, error) {
	...
	var cfgs config
	json.Unmarshal(bytes, &cfgs)
	uin, _ := strconv.ParseInt(cfgs.AccountUin, 10, 64)
	account_status, _ := strconv.Atoi(cfgs.AccountStatus)
	account_relogin_delay, _ := strconv.ParseUint(cfgs.AccountReloginDelay, 10, 64)
	account_relogin_maxtime, _ := strconv.ParseUint(cfgs.AccountReloginMaxTimes, 10, 64)
	account_relogin_interval, _ := strconv.Atoi(cfgs.AccountReloginInterval)
	hartbeat_interval, _ := strconv.Atoi(cfgs.HeartbeatInterval)
	outlog_log_aging, _ := strconv.Atoi(cfgs.OutputLogAging)
	outlog_colorfull := false
	cfb := &config2.Config{
		Account:
		&config2.Account{
			Uin:           uin,
			...
			ReLogin: &config2.Reconnect{
				...
				Delay:    uint(account_relogin_delay),
				MaxTimes: uint(account_relogin_maxtime),
				Interval: account_relogin_interval,
			},
		},
		Heartbeat: struct {
